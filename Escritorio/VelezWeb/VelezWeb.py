...
def register():
    ...
    if alta.validate_on_submit():
        lista = []
        nombre_archivo = 'csv'
        file1 = open(nombre_archivo, "r")
        archivo_csv = csv.reader(file1)
    ...
    if 'username' in session:
        ...
        if reset.validate_on_submit():
            if reset.old.data != passactual:
                ...
                df = pd.read_csv('csv')
                df.loc[df["PASSWORD"]==passactual, "PASSWORD"] = reset.new.data
...
#IMPRESION DE TABLA,EN ESTE CASO UN FIXTURE DE UN TORNEO CON 4 COLUMNAS;EQUIPO,CANCHA,FECHA Y GOLES
...
#BLOQUE PARA DESCARGA DE ARCHIVOS VIA WEB
...
def bajacancha():
    with open('cancha' , 'r+') as a:
        ...
        return send_file('cancha', as_attachment=True ,attachment_filename=tiempoarchivo("cancha.csv" ,'%Y%m%d%H%M%S-{fname}'))
...
def bajaequipo():
    with open('equipo' , 'r+') as a:
        ...
        return send_file('equipo', as_attachment=True ,attachment_filename=tiempoarchivo("equipo.csv" ,'%Y%m%d%H%M%S-{fname}'))
...
def bajafecha():
    with open('fecha' , 'r+') as a:
        ...
        return send_file('fecha', as_attachment=True ,attachment_filename=tiempoarchivo("fecha.csv" ,'%Y%m%d%H%M%S-{fname}'))
...
def bajagoles():
    with open('goles' , 'r+') as a:
        ...
        return send_file('goles', as_attachment=True ,attachment_filename=tiempoarchivo("goles.csv" ,'%Y%m%d%H%M%S-{fname}'))

@app.route('/download/fixture', methods=['GET', 'POST'])
def bajafixture():
    return send_file('agenda', as_attachment=True ,attachment_filename=tiempoarchivo("fixture.csv" ,'%Y%m%d%H%M%S-{fname}'))
